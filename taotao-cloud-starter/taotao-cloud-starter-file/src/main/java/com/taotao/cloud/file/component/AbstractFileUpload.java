...
public abstract class AbstractFileUpload implements FileUpload {
    ...
    @Override
    public FileInfo upload(File file, String fileName) throws Exception {
        FileInfo fileInfo = FileUtil.getFileInfo(file);
        if (!fileInfo.getName().contains(".")) {
            throw new IllegalArgumentException("缺少后缀名");
        } else {
    ...
    @Override
    public FileInfo upload(MultipartFile file, String fileName) throws Exception {
        FileInfo fileInfo = FileUtil.getFileInfo(file);
        if (!fileInfo.getName().contains(FILE_SPLIT)) {
            throw new IllegalArgumentException("缺少后缀名");
        }
    ...
    protected abstract FileInfo uploadFile(File file, FileInfo fileInfo) throws Exception;
